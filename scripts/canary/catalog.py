...
def load_catalog(directory: Path = SCENARIO_DIR) -> dict[str, dict[str, Any]]:
    ...
    expected = {scenario_id for values in TIER_SCENARIOS.values() for scenario_id in values}
    if set(catalog) != expected:
        raise CatalogError(
            f"scenario catalog mismatch: missing={sorted(expected - set(catalog))}, "
            f"unexpected={sorted(set(catalog) - expected)}"
        )
    return catalog
...
def validate_scenario(scenario: object, path: Path) -> None:
    ...
    if not isinstance(scenario["id"], str) or not scenario["id"].startswith("C"):
